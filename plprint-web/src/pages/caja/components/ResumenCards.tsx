...
export default function ResumenCards({ resumen, montoInicial, montoFinalReal }: Props) {
  ...
  const cards = [
    { label: 'Ventas', value: resumen.total_ventas, color: 'text-green-400', bg: 'bg-green-500/10', border: 'border-green-500/30', icon: Wallet },
    { label: 'Ingresos', value: resumen.total_ingresos, color: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/30', icon: ArrowUpRight },
    { label: 'Gastos', value: resumen.total_gastos, color: 'text-red-400', bg: 'bg-red-500/10', border: 'border-red-500/30', icon: ArrowDownRight },
    { label: 'Retiros', value: resumen.total_retiros, color: 'text-orange-400', bg: 'bg-orange-500/10', border: 'border-orange-500/30', icon: Landmark },
  ];
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
      ...
      <div className="border border-blue-500/30 bg-blue-500/10 rounded-xl px-4 py-3 flex flex-col gap-1">
        <span className="text-xs text-muted-foreground flex items-center gap-1.5">
          <Wallet size={13} className="text-blue-400" /> Efectivo esperado
        </span>
      ...
      <div className={`border rounded-xl px-4 py-3 flex flex-col gap-1 ${
        ...
        <span className="text-xs text-muted-foreground flex items-center gap-1.5">
          <Wallet size={13} className={diferencia !== undefined ? (diferencia >= 0 ? 'text-green-400' : 'text-red-400') : 'text-muted-foreground'} /> Diferencia
        </span>
