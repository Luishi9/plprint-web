...
import type { ImportInsumoConfirmResult } from '@/types/insumo.types';

interface ImportInsumosDoneViewProps {
  result: ImportInsumoConfirmResult;
  onClose: () => void;
}

export function ImportInsumosDoneView({ result, onClose }: ImportInsumosDoneViewProps) {
  return (
        ...
        <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-4">
          ...
          <ul className="space-y-1 max-h-32 overflow-y-auto">
            {result.errores.map((e, i) => (
              <li key={e.codigo || `error-${i}`} className="text-xs text-red-300">Fila {e.fila} Código {e.codigo || '(sin codigo)'}: {e.razon}</li>
            ))}
          </ul>
        </div>
      )}
